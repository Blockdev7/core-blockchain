...
startValidator(){
  ...
  while [[ $i -le $totalValidator ]]; do
    ...
    if tmux has-session -t node$i > /dev/null 2>&1; then
        ...
        tmux new-session -d -s node$i
        tmux send-keys -t 0 "./node_src/build/bin/geth --datadir ./chaindata/node$i --networkid $CHAINID --bootnodes $BOOTNODE --mine --port 326$j --nat extip:$IP --gpo.ignoreprice 0 --unlock 0 --password ./chaindata/node$i/pass.txt --syncmode=full --gcmode=archive console" Enter
    fi
